...
func (c *CompositeRetryPolicy) SetPolicies(rtyPolicy []gretry.RtyPolicy) {
	c.Policies = rtyPolicy
}

func (c *CompositeRetryPolicy) CanRetry(ctx gretry.RtyContext) bool {
...
func (c *CompositeRetryPolicy) Open(parent gretry.RtyContext) gretry.RtyContext {
	var list []gretry.RtyContext
...
func (c *CompositeRetryPolicy) Close(ctx gretry.RtyContext) {
	var ctxs = (ctx.(*CompositeRetryContext)).Contexts
	var polices = (ctx.(*CompositeRetryContext)).Policies
	for i := 0; i < len(ctxs); i++ {
...
func (c *CompositeRetryPolicy) RegisterError(ctx gretry.RtyContext, err error) {
	var ctxs = (ctx.(*CompositeRetryContext)).Contexts
	var polices = (ctx.(*CompositeRetryContext)).Policies
	for i := 0; i < len(ctxs); i++ {
...
type CompositeRetryContext struct {
	Contexts []gretry.RtyContext
	Policies []gretry.RtyPolicy
	context.RtyContextSupport
	gretry.SimpleAttributeAccessorSupport
}

func NewCompositeRetryContext(parent gretry.RtyContext, contexts []gretry.RtyContext, policies []gretry.RtyPolicy) *CompositeRetryContext {
